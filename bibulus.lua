            ...
            elseif WeaponType == 3 then
                if RSEquipped:FindFirstChild("Backstab") then
                    ...
                    for _,v in BackstabY do
                        if (LocalPlayer.Character.Hitbox.Position - (Player.Hitbox.CFrame * CFrame.new(0,0,4.25)).Position + Vector3.new(0,v,0)).Magnitude <= 4 then
                            AllowedShoot = true
                        end
                    end
                else
...
namecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    ...
    if not Library.Unloaded then
        if Method == "Raycast" and AimPosition and Script == script then
